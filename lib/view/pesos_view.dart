...
class PesosView extends StatefulWidget {
  const PesosView({super.key});

  @override
  State<PesosView> createState() => _PesosViewState();
}

class _PesosViewState extends State<PesosView> {
  ...
  void delteTable(GranjaProvider ProvGranja, DetallePesoDivision data) {
    setState(() {
      ProvGranja.listaDetallesPesaje.remove(data);
      for (int i = 0; i < ProvGranja.listaDetallesPesaje.length; i++) {
        ProvGranja.listaDetallesPesaje[i] = DetallePesoDivision(
          ProvGranja.listaDetallesPesaje[i].id,
          ProvGranja.listaDetallesPesaje[i].peso,
          ProvGranja.listaDetallesPesaje[i].sexo,
          ProvGranja.listaDetallesPesaje[i].ranchoDivisionId,
          ProvGranja.listaDetallesPesaje[i].estatus,
          ProvGranja.listaDetallesPesaje[i].createdAt,
          ProvGranja.listaDetallesPesaje[i].updatedAt,
          i,
        );
      }
      pesoKG.clear();
    ...
    if (sexoAnimal == null || pesoKG.text.isEmpty) {
      ...
      setState(() {
        if (filaSeleccionada == -1) {
          ProvGranja.listaDetallesPesaje.add(
            DetallePesoDivision(
              ProvGranja.listaDetallesPesaje.length,
              double.parse(pesoKG.text),
              sexoAnimal!,
              ProvGranja.listaDetallesPesaje.length,
              true,
              _fechaElegida,
          ...
          ProvGranja.listaDetallesPesaje[filaSeleccionada] =
              DetallePesoDivision(
            ProvGranja.listaDetallesPesaje[filaSeleccionada].id,
            double.parse(pesoKG.text),
            sexoAnimal!,
            ProvGranja.listaDetallesPesaje[filaSeleccionada].ranchoDivisionId,
            ProvGranja.listaDetallesPesaje[filaSeleccionada].estatus,
            ProvGranja.listaDetallesPesaje[filaSeleccionada].createdAt,
            ProvGranja.listaDetallesPesaje[filaSeleccionada].updatedAt,
            filaSeleccionada,
          );
          filaSeleccionada = -1;
        }
  ...
  Widget build(BuildContext context) {
    ...
    if (modificacion) {
      setState(() {
        _fechaElegida = ProvGranja.pesajeSeleccionado["fecha"];
        tipoSeleccionado = ProvGranja.listaTipoPesaje.firstWhere(
          (objeto) =>
              objeto.id == ProvGranja.pesajeSeleccionado['tipoPesajeId'],
        );
        divisionSeleccionado = ProvGranja.listaDivisionesRancho.firstWhere(
          (objeto) =>
              objeto.id == ProvGranja.pesajeSeleccionado['ranchoDivicionId'],
        );
      });
      modificacion = false;
    }
    ...
    return Scaffold(
            ...
            onPressed: ProvGranja.botonActivo
                ...
                : () {
                    if (tipoSeleccionado == null ||
                        ProvGranja.listaDetallesPesaje.isEmpty ||
                        divisionSeleccionado == null) {
                      const snackBar = SnackBar(
                        content:
                            Text('Campo de tipo, divición o pesajes vacio.'),
                        duration: Duration(seconds: 1, milliseconds: 500),
                      );
                      ScaffoldMessenger.of(context).showSnackBar(snackBar);
                      ProvGranja.botonActivo = false;
                    } else {
                      PesosController().updatePesaje(
                        ProvGranja,
                        _fechaElegida,
                        tipoSeleccionado,
                      ...
                      setState(() {
                        PotreroController()
                            .datoPesaje(ProvGranja, context, false);
                      });
                    }
                  },
            icon: const Icon(
      ...
      /*creamos el formulario de nuevo pesaje 
      *contiene:
      ...
      * una tabla para mostrar los pesajes ingresados
      */
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButton<TipoPesaje>(
              ...
              items: ProvGranja.listaTipoPesaje
                  .map<DropdownMenuItem<TipoPesaje>>((TipoPesaje value) {
                ...
                ElevatedButton.icon(
                  style: ButtonStyle(
                    backgroundColor: MaterialStateProperty.all<Color>(Colors
                        .transparent), // Cambia "Colors.blue" por el color deseado
                  ),
                  onPressed: () {
                    seleccionarDato();
                  },
              ...
              child: ListView(
                children: <Widget>[
                  DataTable(
                    ...
                    rows: ProvGranja.listaDetallesPesaje.map(
                      (data) {
                        return DataRow(
                          cells: [
                            DataCell(
                              ...
                              ButtonBar(
                                ...
                                children: [
                                  IconButton(
                                    onPressed: () {
                                      setState(() {
                                        pesoKG.text = data.peso.toString();
                                        sexoAnimal = data.sexo;
                                        filaSeleccionada = data.posicion;
                                      });
                                    },
